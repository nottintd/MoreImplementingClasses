...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        self.h_rect = rg.Rectangle(rg.Point(intersection_center.x - (width / 2), intersection_center.y -
                                            (letter_thickness / 2)),
                              rg.Point(intersection_center.x + (width / 2), intersection_center.y +
                                       (letter_thickness / 2)))

        self.v_rect = rg.Rectangle(rg.Point(intersection_center.x - (letter_thickness / 2), intersection_center.y -
                                            (letter_thickness / 2)),
                                   rg.Point(intersection_center.x + (letter_thickness / 2), intersection_center.y +
    ...
    def move_by(self, dx, dy):
        ...
        # self.v_rect = rg.Rectangle(rg.Point(self.v_rect.get_upper_left_corner().x + dx,
        ...
        #
        # self.h_rect = rg.Rectangle(rg.Point(self.h_rect.get_upper_left_corner().x + dx,
    ...
    def clone(self):
        ...
        return new_t
        # return rg.Rectangle(self.h_rect.corner_1, self.h_rect.corner_2), \
        #        rg.Rectangle(self.v_rect.corner_1, self.v_rect.corner_2)
        # --------------------------------------------------------------
        # DONE: 7.
        #   READ the above specification, including the Example.
        #   Implement and test this method by uncommenting the appropriate
